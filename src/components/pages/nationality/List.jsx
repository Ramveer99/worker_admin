...
function List() {
    ...
    const [deleteModelMessage, setDeleteModelMessage] = useState('Are you sure want to delete this nationality?');
    ...
    const columns = [
        ...
        {
            name: 'Nationality',
            selector: row => row.nationality_name,
            sortable: true,
        },
        // {
        //     name: 'Description',
        ...
        },
        {
            ...
            cell: row => (
                <div>
                    <Link to={`/nationality/edit/${row._id}`}>
    ...
    const submitDelete = async () => {
        ...
        try {
            let res = await axios.delete(`admin/nationalitydel/${idBeingDeleting}`)
    ...
    const handleDeleteConfirm = (row, type) => {
        ...
        setDeleteModelMessage(`Are you sure want to ${type} the  ${row?.nationality_name} Nationality?`)
    ...
    const getNationalityList = useCallback(async () => {
        try {
            setLoading(true);
            let res = await axios.get(`admin/nationalitylist?page=${pageNumber}&keyword=${searchKeyWord}&per_page=${perPage}&sort_by=${sortField}&sort_order=${sortDirection}`)
            // console.log(res);
            setNationalityData(res.data.result.nationalitydata)
    ...
    }, [getNationalityList, searchKeyWord, pageNumber, sortField, perPage, location])
    return (
        <>
            <Helmet>
                <title>Nationalities Management</title>
            </Helmet>
            <LayoutPage>
                <div className="row">
                        ...
                        <div className="card my-4">
                            <div className="card-header p-0 position-relative mt-n4 mx-3 z-index-2">
                                <div className="bg-gradient-primary shadow-primary border-radius-lg pt-4 pb-3">
                                    <h6 className="text-white text-capitalize ps-3 custom-card-heading">Nationalities</h6>
                                    <Link to="/nationality/addnew" title='Add New' className='btn btn-rounded btn-icon btn-primary custom-add-new-button'><i className='fa fa-plus'></i></Link>
                                </div>
                            </div>
