...
function List() {
    ...
    const columns = [
        ...
        },
        {
            ...
            cell: row => (
                <div>
                    <Link to={`/involvements/edit/${row._id}`}>
    ...
    const submitDelete = async () => {
        ...
        try {
            let res = await axios.delete(`admin/involvementdel/${idBeingDeleting}`)
    ...
    const handleDeleteConfirm = (row, type) => {
        ...
        setDeleteModelMessage(`Are you sure want to ${type} this involvement?`)
    ...
    const getResponsibilitiesList = useCallback(async () => {
        try {
            setLoading(true);
            let res = await axios.get(`admin/involvements?page=${pageNumber}&keyword=${searchKeyWord}&per_page=${perPage}&sort_by=${sortField}&sort_order=${sortDirection}`)
                        ...
                        <div className="card my-4">
                            <div className="card-header p-0 position-relative mt-n4 mx-3 z-index-2">
                                <div className="bg-gradient-primary shadow-primary border-radius-lg pt-4 pb-3">
                                    <h6 className="text-white text-capitalize ps-3 custom-card-heading">Involvements</h6>
                                    <Link to="/involvements/addnew" title='Add New' className='btn btn-rounded btn-icon btn-primary custom-add-new-button'><i className='fa fa-plus'></i></Link>
                                </div>
                            </div>
