...
function List() {
    ...
    const [deleteModelMessage, setDeleteModelMessage] = useState('Are you sure want to delete this jobtitle?');
    ...
    const columns = [
        ...
        {
            name: 'Created Date',
            selector: row => moment(row.created_at).format('DD-MMM-YYYY HH:MM A'),
    ...
    const submitDelete = async () => {
        ...
        console.warn("delete---->",idBeingDeleting)
        setLoadingDeleteModelConfirmText('Deleting')
        //  activate deactivate user
        try {
            let res = await axios.delete(`/admin/commentdel/${idBeingDeleting}`)
    ...
    const handleDeleteConfirm = (row, type) => {
        ...
        setDeleteModelMessage(`Are you sure want to ${type} this Comment?`)
    ...
    const getCategoryList = useCallback(async () => {
        try {
            setLoading(true);
            let res = await axios.get(`admin/commentdisplay?page=${pageNumber}&keyword=${searchKeyWord}&per_page=${perPage}&sort_by=${sortField}&sort_order=${sortDirection}`)
            console.log("comment api data----->",res.data.result)
            setCategoryData(res.data.result.commentdata)
                        ...
                        <div className="card my-4">
                            <div className="card-header p-0 position-relative mt-n4 mx-3 z-index-2">
                                <div className="bg-gradient-primary shadow-primary border-radius-lg pt-4 pb-3">
                                    <h6 className="text-white text-capitalize ps-3 custom-card-heading">Comment List</h6>
                                    {/* <Link to="/jobtitle/addnew" title='Add New' className='btn btn-rounded btn-icon btn-primary custom-add-new-button'><i className='fa fa-plus'></i></Link> */}
                                </div>
                            </div>
